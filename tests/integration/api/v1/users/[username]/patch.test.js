...
describe("PATCH /api/v1/users/[username]", () => {
  describe("Anonymous user", () => {
    ...
    test("With duplicated username", async () => {
      const user1Response = await fetch("http://0.0.0.0:3000/api/v1/users", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          password: "user1senha123",
        }),
      });

      expect(user1Response.status).toBe(201);

      const user2Response = await fetch("http://0.0.0.0:3000/api/v1/users", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          ...
          password: "user2senha123",
        }),
      });

      expect(user2Response.status).toBe(201);

      const response = await fetch("http://0.0.0.0:3000/api/v1/users/user2", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    test("With duplicated email", async () => {
      const user1Response = await fetch("http://0.0.0.0:3000/api/v1/users", {
        ...
        body: JSON.stringify({
          username: "email1",
          email: "email1-email@email.com",
          password: "email1senha123",
        }),
      });
      ...
      const user2Response = await fetch("http://0.0.0.0:3000/api/v1/users", {
        ...
        body: JSON.stringify({
          username: "email2",
          email: "email2-email@email.com",
          password: "email2senha123",
        }),
      });

      expect(user2Response.status).toBe(201);

      const response = await fetch("http://0.0.0.0:3000/api/v1/users/email2", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
